...
async function sendOwnerNotification(request, baseUrl) {
  ...
  const body = `
    <p style="margin:0 0 12px;">Nouvelle demande de devis <b>${escapeHtml(request.ref || '')}</b>.</p>
  ...
  await getTransport().sendMail({
    ...
    subject: `Nouvelle demande de devis — ${c.name || 'client'} (${request.ref || ''})`,
    html: wrap('Nouvelle demande de devis', body)
  });
}

async function sendDevisEmail(request, settings, reply, pdfBuffer) {
  const c = request.client || {};
  ...
  const body = `
    ...
    <p style="margin:0 0 8px;color:#6F6455;font-size:13px;">Votre devis <b>${escapeHtml(reply.quoteNumber || request.ref || '')}</b> est joint en PDF. Récapitulatif :</p>
    ...
    ${q.depositPct > 0 ? `<p style="margin:0 0 16px;text-align:right;color:#6F6455;font-size:13px;">Acompte (${q.depositPct}%) : ${euros(q.deposit)}</p>` : ''}
    <p style="margin:16px 0 0;color:#6F6455;font-size:12px;">Ce devis est gratuit et sans engagement. Pour toute question, répondez simplement à cet e-mail.</p>`;
  await getTransport().sendMail({
...
module.exports = { mailReady, ownerAddress, getTransport, sendOwnerNotification, sendDevisEmail };
